...
- **Perfil de estudiante/egresado** (colección `estudiantes`, JSON de `scripts/fetch-estudiantes.mjs`): misma retícula del perfil de docente — foto + ficha (vinculación, cohorte, grupo, director/codirector enlazados) y redes académicas; propuesta/proyecto/tesis doctoral citada en serif. Los slugs que ya son docentes se omiten (el perfil de docente manda en esa URL).
- **Formulario de contacto**: banda page-hero con dek serif; campos con borde fino, placeholder, hover que oscurece y foco con borde carmesí + anillo `--c-crimson-soft`; fila doble nombre/correo; tarjeta de datos sticky con filas de regla fina; botón primario con flecha ↗. Los avisos de validación se reemplazan por mensajes propios en español (los nativos salen en el idioma del navegador, no en el de la página). El envío es asíncrono y el resultado se anuncia en la misma página (`.form-estado`, `role=status`); si falla, se ofrece el correo y el teléfono del programa. Verificable con `node scripts/probar-form.mjs`.
  - ⚠️ **Pendiente para que el formulario funcione**: reemplazar `ACCESS_KEY` en `contactenos.astro` por la clave real de [Web3Forms](https://web3forms.com) (gratuita; se registra con `doctoradoing@udistrital.edu.co` y los mensajes llegan a ese correo). Mientras diga `PENDIENTE_ACCESS_KEY`, el envío falla de forma controlada y muestra el aviso de respaldo.
  - **Protección contra bots**, en capas de menor a mayor eficacia:
    1. *Honeypots* (`botcheck`, validado por Web3Forms, y `sitio_web`, validado en el cliente): fuera del alcance del teclado y de lectores de pantalla; si vienen llenos se finge éxito y no se envía nada. Atrapan bots que rellenan todo por defecto.
    2. *Trampa de tiempo*: se rechaza el envío en menos de 3 s desde la carga (una persona no llena el formulario tan rápido). Fallo suave: el usuario reintenta y funciona.
    3. *Control de reenvío*: 15 s entre envíos, evita ráfagas y el doble clic accidental.
    4. ⚠️ **Cloudflare Turnstile — la única defensa real contra bots guiados por IA.** Las capas 1–3 son del lado del cliente: un bot con navegador automatizado (o alguien que copie la `access_key` del HTML y haga POST directo) las salta todas. Está cableado y se activa solo con poner la clave en `TURNSTILE_KEY` (gratuito, casi siempre invisible para el usuario). **Además, restringir el dominio en el panel de Web3Forms** para que la clave no sirva desde otros sitios.
